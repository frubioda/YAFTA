...
; 
; PURPOSE: Uses IDL's PLOTS.PRO to overlay features' boundaries, colored
...
;   07/08/05, BTW: Added edgepix keyword to return edge pixels' addresses
;
;- 
...
for i=0,n_right-1 do begin
    ...
    plots, $
      [rightx(i)+0.5-dx,rightx(i)+0.5-dx],[righty(i)-.5+dx,righty(i)+.5], $
...
for i=0,n_left-1 do plots, $
  [leftx(i)-0.5+dx,leftx(i)-0.5+dx],[lefty(i)-.5,lefty(i)+.5-dx], $
  color=(dcolor*abmask(left(i)))mod(max_col-min_col) + min_col,thick=thick
...
for i=0,n_upper-1 do plots, $
  [upperx(i)-.5,upperx(i)+.5-dx],[uppery(i)+0.5-dx,uppery(i)+0.5-dx], $
  color=(dcolor*abmask(upper(i)))mod(max_col-min_col) + min_col,thick=thick
...
for i=0,n_bottom-1 do plots, $
  [bottomx(i)-.5+dx,bottomx(i)+.5],[bottomy(i)-0.5+dx,bottomy(i)-0.5+dx], $
  color=(dcolor*abmask(bottom(i)))mod(max_col-min_col) + min_col,thick=thick
