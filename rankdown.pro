...
; 
; PURPOSE: A flux-ranked, downhill labelling algorithm that
;          groups collections of pixels together that are
;          convex in absolute field strength.  Simply put,
;          pixels that lie on the same ``hills'' in absolute
;          field strength are grouped together.
;
; ARGUMENTS:
...
;          not grouped, and receive a zero label; DEFAULT = 30
...
for q2 = 0,nloop-1 do begin
	...
	tempmask = intarr(nx,ny)
	tempmask(ranks) = indgen(nx*ny)+1 
