...
;     27 Jun 2005 --- BTW: Bug on found on DEC architectures.  IDL will 
...
if not(keyword_set(min_size)) then begin
    min_size = 10
...
for j = 1,n_lab-1 do begin
	...
	;===========
        ...
        ;=====================
        vx_j  = total(vx  *img(where_j))/(tot_j)
        vx2_j = total((vx-vx_j)^2*img(where_j))/(tot_j)
        vy_j  = total(vy  *img(where_j))/(tot_j)
        vy2_j = total((vy-vy_j)^2*img(where_j))/(tot_j)
        vxvy_j = total((vx-vx_j) * (vy-vy_j) * img(where_j))/(tot_j)
