...
;	RANKDOWN.PRO, ALT_MASK.PRO, respectively, and then 
;	CREATE_FEATURES.PRO), and stores this info as strings
; 	in the "ALT_LABEL" fields of each of the convex and 
;	contiguous features' structures.  Each convex grouping 
;	(a "nation") should be co-spatial with a sub-region of 
;	exactly one contiguous grouping (a "continent"), while each 
;	contiguous grouping might contain many convex groupings, 
;	and/or zeros (since since a minimum size criterion might 
;	prevent the creation of convex features where a contiguous 
;	feature exists).
;
; ARGUMENTS:
